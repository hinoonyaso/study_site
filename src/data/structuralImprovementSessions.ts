...
def test_cancel_sequence_contains_goal_state_check():
  ...
  },
  {
    ...
    part: "Part 11. 프롬프트/컨텍스트/하네스 엔지니어링",
...
export const promptHarnessSessions: Session[] = structuralSessions.filter((session) =>
  session.part === "Part 11. 프롬프트/컨텍스트/하네스 엔지니어링",
);
...
export const structuralQualityRemediationChecklist = [
  ...
  },
  {
    ...
    evidenceFiles: ["src/components/visualizers/VisualizerHub.tsx", "src/data/sessionAdapter.ts"],
    evidenceSessionIds: ["fk_matrix_ik_singularity_visual_lab"],
    evidenceVisualizationIds: ["vis_fk_matrix_ik_singularity_steps"],
  },
  {
    ...
    status: "resolved",
    remediation: "Part 11을 추가해 prompt template, retrieval budget, schema validation, golden eval을 가르친다.",
    evidenceFiles: ["src/data/curriculumV2.ts", "src/data/structuralImprovementSessions.ts"],
